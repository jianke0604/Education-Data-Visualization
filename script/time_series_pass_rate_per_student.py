...
def load_id():
    global title_id, student_id, ans, pre_prefix_submit, pre_prefix_pass, total_try, total_pass
    ...
    pre_prefix_submit = {i: {} for i in student_id}
    total_try = {i: {} for i in student_id}
    total_pass = {i: {} for i in student_id}
...
def deal():
    global pre_prefix_submit, pre_prefix_pass
    for _, row in merged_dataframe.iterrows():
        student_id = row['student_ID']
        date = row['date']
        date = date.strftime('%Y-%m-%d')
        print(f"student_id: {student_id}, date: {date}")
        if date not in pre_prefix_submit[student_id]:
            pre_prefix_submit[student_id][date] = 1
        else:
            pre_prefix_submit[student_id][date] += 1
        
        if row['state'] == 'Absolutely_Correct':
            if date not in pre_prefix_pass[student_id]:
                pre_prefix_pass[student_id][date] = 1
            else:
                pre_prefix_pass[student_id][date] += 1
        else: 
            if date not in pre_prefix_pass[student_id]:
                pre_prefix_pass[student_id][date] = 0


def get_ans():
    global pre_prefix_submit, pre_prefix_pass, ans, total_try, total_pass
    ...
    for student_id in pre_prefix_submit:
        ...
        for date in date_range:
            ...
            if date_str not in pre_prefix_submit[student_id]:
                ans[student_id][date_str] = 0 if submit_cnt == 0 else pass_cnt / submit_cnt
            else:
                submit_cnt += pre_prefix_submit[student_id][date_str]
                pass_cnt += pre_prefix_pass[student_id][date_str]
                print(f"submit_cnt: {submit_cnt}, pass_cnt: {pass_cnt}, date: {date} ")
                ans[student_id][date_str] = 0 if submit_cnt == 0 else pass_cnt / submit_cnt

            total_try[student_id][date_str] = submit_cnt
            total_pass[student_id][date_str] = pass_cnt
...
if __name__ == '__main__':
    ...
    with open('../data/time_series_pass_rate_per_student.json', 'w') as f:
        json.dump(ans, f, indent=4)

    with open('../data/prefix_sum_try_per_student.json', 'w') as f:
        json.dump(total_try, f, indent=4)

    with open('../data/prefix_sum_pass_per_student.json', 'w') as f:
        json.dump(total_pass, f, indent=4)